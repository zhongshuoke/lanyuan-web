...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#exportExcel").click("click", function() {//绑定查询按扭
			...
			f.attr('action','${pageContext.request.contextPath}/background/wxrecommarticle/exportExcel.html');
			f.submit();
		});
		$("#add").click("click", function() {//绑定查询按扭
			...
			dialog = parent.$.ligerDialog.open({
				width : 1000,
				height : 1100,
				url : rootPath + '/background/wxrecommarticle/addUI.html?articleId='+cbox,
				title : "查看文章",
		...
		$("#editView").click("click", function() {//绑定查询按扭
			...
			dialog = parent.$.ligerDialog.open({
				width : 1000,
				height : 1100,
				url : rootPath + '/background/wxrecommarticle/editUI.html?articleId='+cbox,
				title : "微信文字编辑",
				isHidden : false
			});
		});
		...
		$("#perrole").click("click", function() {//绑定查询按扭
			...
			dialog = parent.$.ligerDialog.open({
				width : 500,
				height : 410,
				url : rootPath + '/background/wxrecommarticle/accRole.html?id='+cbox.id+'&accountName='+encodeURI(encodeURI(cbox.accountName))+'&roleName='+encodeURI(encodeURI(cbox.roleName)),
				title : "分配角色",
				isHidden : false
			});
		});
			...
			parent.$.ligerDialog.confirm('删除后不能恢复，确定删除吗？', function(confirm) {
				if (confirm) {
					$.ajax({
					    ...
					    url: rootPath + '/background/wxrecommarticle/deleteById.html', //要访问的后台地址

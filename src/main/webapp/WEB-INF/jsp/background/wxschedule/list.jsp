	...
	$(function() {
		grid = window.lanyuan.ui.lyGrid({
					...
					l_column : [ {
						...
						colkey : "month_of_year",
						name : "一年第几月"
					} ],
					jsonUrl : '${pageContext.request.contextPath}/background/wxschedule/query.html',
					checkbox : true
				});
		...
		$("#exportExcel").click("click", function() {//绑定查询按扭
			...
			f.attr('action','${pageContext.request.contextPath}/background/wxschedule/exportExcel.html');
			f.submit();
		});
		$("#add").click("click", function() {//绑定查询按扭
			dialog = parent.$.ligerDialog.open({
				width : 300,
				height : 310,
				url : rootPath + '/background/wxschedule/addUI.html',
				title : "增加计划",
		...
		$("#editView").click("click", function() {//绑定查询按扭
			...
			dialog = parent.$.ligerDialog.open({
				width : 300,
				height : 310,
				url : rootPath + '/background/wxschedule/editUI.html?accountId='+cbox,
				title : "修改计划",
				isHidden : false
			});
		});
		$("#perrole").click("click", function() {//绑定查询按扭
			...
			dialog = parent.$.ligerDialog.open({
				width : 500,
				height : 410,
				url : rootPath + '/background/wxschedule/accRole.html?id='+cbox.id+'&accountName='+encodeURI(encodeURI(cbox.accountName))+'&roleName='+encodeURI(encodeURI(cbox.roleName)),
				title : "分配角色",
				isHidden : false
			});
		});
			...
			parent.$.ligerDialog.confirm('删除后不能恢复，确定删除吗？', function(confirm) {
				if (confirm) {
					$.ajax({
					    ...
					    url: rootPath + '/background/wxschedule/deleteById.html', //要访问的后台地址
	...
	<div class="divBody">
		<div class="search">
			<form name="fenye" id="fenye">
				名称：<input type="text" name="hour" value="${param.name}"
